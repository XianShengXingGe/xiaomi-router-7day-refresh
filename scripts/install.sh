...
download_file() {
  ...
  if command -v curl >/dev/null 2>&1; then
    curl -fLk --connect-timeout 4 -m 60 -o "$out" "$url" 2>/dev/null && [ -s "$out" ] && return 0
...
download_with_fallback() {
  url="$1"; out="$2"
  # Try fast domestic mirror nodes first
  for mirror in "https://ghfast.top" "https://gh.ddlc.top" "https://ghproxy.net"; do
    mirror_url="$mirror/$url"
    note "$(msg "尝试镜像加速: $mirror" "Trying mirror: $mirror")"
    if download_file "$mirror_url" "$out"; then return 0; fi
  done

  # Direct official GitHub fallback
  note "$(msg "尝试直连官方 GitHub: $url" "Trying direct GitHub: $url")"
